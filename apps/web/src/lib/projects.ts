...
export type Project = {
  ...
  cardImageUrl?: string;
  gallery: GalleryItem[];
  galleryHeadings: { first: string; second: string; third: string };
  galleryNote: string;
...
function toProject(doc: PayloadProject): Project {
  return {
    ...
    galleryHeadings: {
      first: doc.galleryHeadings?.first ?? "",
      second: doc.galleryHeadings?.second ?? "",
      third: doc.galleryHeadings?.third ?? "",
    },
    galleryNote: doc.galleryNote ?? "",
    challenge: {
